...
contract VoluntaryRealNameChallengeModule is IDisclosurePolicyModule {
    DisclosurePolicyRegistry public immutable disclosureRegistry;
    uint256 public immutable realNameChallengePolicyId;

    constructor(DisclosurePolicyRegistry disclosureRegistry_, uint256 realNameChallengePolicyId_) {
        if (realNameChallengePolicyId_ == 0) revert AVADataTypes.EmptyValue();
        disclosureRegistry = disclosureRegistry_;
        realNameChallengePolicyId = realNameChallengePolicyId_;
    }
    ...
    function validateDisclosureForAction(
        ...
        if (_isChallengeAction(action)) {
            if (disclosurePolicyId != realNameChallengePolicyId || stage != AVADataTypes.AVAStage.Verification) {
